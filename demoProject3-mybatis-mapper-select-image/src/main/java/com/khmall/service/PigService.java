...
public class PigService {
	...
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		...
		File imgFolder = new File(uploadDir + "cute/");
		File imgFile = new File(imgFolder + fileName);
		
		// 나중에 이미지 파일을 저장하는 폴더가 존재하지 않을 경우 생성하는 코드
		// 우리가 저장하고자 하는 파일 경로 설정
		
		if( !imgFolder.exists()  /*만약에 이미지 폴더가 존재하지 않을 때가 true인 ! 추가*/) {
			//폴더 생성하기
			imgFolder.mkdirs(); //폴더가 존재하지 않을 경우 우리가 지정한 폴더에 생성
		}
		...
		try {
			...
			pig.setPig_image_path(imgFolder + "/"+fileName);  //-> data에 /images/파일명.jpg
